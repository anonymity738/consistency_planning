...
class TrainLoop:
    def __init__(
        ...
        self.data = data
        self.dataloader = cycle(th.utils.data.DataLoader(
            data, batch_size=batch_size, num_workers=1, shuffle=True, pin_memory=True
        ))
    ...
    def run_loop(self):
        while not self.lr_anneal_steps or self.step < self.lr_anneal_steps:
            if self.use_history:
                batch, cond, returns, weight_map = next(self.dataloader)
                self.run_step(batch, cond, returns, weight_map)
            else:
                batch, cond, returns = next(self.dataloader)
                self.run_step(batch, cond, returns)
    ...
    def run_step(self, batch, cond, returns, weight_map=None):
        self.forward_backward(batch, cond, returns, weight_map)
    ...
    def forward_backward(self, batch, cond, returns, weight_map=None):
        self.mp_trainer.zero_grad()
        for i in range(0, batch.shape[0], self.microbatch):
            micro = batch[i : i + self.microbatch].to(dist_util.dev())
            if type(cond) is th.Tensor:
                ...
                micro_cond = None
            micro_returns = returns[i : i + self.microbatch].to(dist_util.dev())
            micro_weight_map = None
            if weight_map is not None: micro_weight_map = weight_map[i : i + self.microbatch].to(dist_util.dev())
...
class CMTrainLoop(TrainLoop):
    ...
    def run_step(self, batch, cond, returns):
        self.forward_backward(batch, cond, returns)
        took_step = self.mp_trainer.optimize(self.opt)
        if took_step:
            self._update_ema()
    ...
    def forward_backward(self, batch, cond, returns):
        self.mp_trainer.zero_grad()
        for i in range(0, batch.shape[0], self.microbatch):
            micro = batch[i : i + self.microbatch].to(dist_util.dev())
            micro_cond = {
            ...
            if self.training_mode == "progdist":
                ...
                compute_losses = functools.partial(
                    self.diffusion.latent_consistency_losses,
                    self.ddp_model,
                    micro,
                    num_scales,
                    target_model=self.target_model,
